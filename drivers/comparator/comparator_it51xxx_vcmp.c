...
#include <zephyr/drivers/comparator/it51xxx_vcmp.h>
...
static void it51xxx_vcmp_trigger_work_handler(struct k_work *item)
{
	...
	if (data->user_cb) {
		data->user_cb(data->vcmp, data->user_cb_data);
	}
}
...
static int it51xxx_vcmp_set_trigger(const struct device *dev, enum comparator_trigger trigger)
{
	...
	case COMPARATOR_TRIGGER_BOTH_EDGES:
		LOG_ERR("IT51XXX comparator does not support trigger "
...
static int it51xxx_vcmp_trigger_is_pending(const struct device *dev)
{
	...
	LOG_ERR("IT51XXX comparator does not support function trigger_is_pendingt");
	return -ENOTSUP;
}
...
static void vcmp_it51xxx_isr(const struct device *dev)
{
	 ...
	 *  ture (1): Greater than CMPnTHRDAT [9:0]
	 */
	bool comparator_mode;

	/* Find out which voltage comparator triggered */
	for (int idx = VCMP_CHANNEL_0; idx < VCMP_CHANNEL_CNT; idx++) {
		status = (sys_read8(reg_base + vcmp_status_ctrl_reg[idx]) & VCMP_CMPXRTIS);
		if (status) {

			vcmp_enable(dev, false);

			comparator_mode =
				sys_read8(base_ch + REG_VCMP_CH_CTL) & VCMP_GREATER_THRESHOLD;
			...
			if ((comparator_mode &&
				...
				if (vcmp_work_addr[idx]) {
					...
					k_work_submit_to_queue(
						work_q, (struct k_work *)vcmp_work_addr[idx]);
#else
...
static int vcmp_it51xxx_init(const struct device *dev)
{
	...
	/* Set voltage comparator to Edge-sensed mode */
	sys_write8(sys_read8(base_ch + REG_VCMP_CH_CTL) | VCMP_CMP_EDGE_SENSED_MODE,
		   base_ch + REG_VCMP_CH_CTL);
	...
	sys_write8(sys_read8(base_ch + REG_VCMP_CH_CTL) |
			   (data->adc_ch_cfg.channel_id & VCMP_CHANNEL_ID_REG_MASK),
		   base_ch + REG_VCMP_CH_CTL);
	...
	err = vcmp_set_threshold(dev, config->threshold_mv);
	if (err) {
		return err;
	}
...
#define VCMP_IT51XXX_INIT(inst)                                                                    \
	...
	DEVICE_DT_INST_DEFINE(inst, vcmp_it51xxx_init, NULL, &vcmp_it51xxx_data_##inst,            \
			      ...
			      CONFIG_VCMP_IT51XXX_INIT_PRIORITY, &it51xxx_vcmp_api);

DT_INST_FOREACH_STATUS_OKAY(VCMP_IT51XXX_INIT)
#ifdef CONFIG_VCMP_IT51XXX_WORKQUEUE
BUILD_ASSERT(CONFIG_COMPARATOR_INIT_PRIORITY < CONFIG_VCMP_IT51XXX_INIT_PRIORITY,
	     "CONFIG_VCMP_IT51XXX_INIT_PRIORITY must not be greater than "
